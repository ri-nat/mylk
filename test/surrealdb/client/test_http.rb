...
class TestHTTP < Minitest::Test
  ...
  def test_it_connects_to_surrealdb
    assert SurrealDB::Client::HTTP.new(DATABASE_URL).connect
  end

  def test_it_executes_successfully
    client = SurrealDB::Client::HTTP.new(DATABASE_URL).tap(&:connect)

    assert client.execute("INFO FOR DB").success?
  end

  def test_it_connects_automatically_on_execute
    assert SurrealDB::Client::HTTP.new(DATABASE_URL).execute("INFO FOR DB").success?
  end
end
